...
<body>
<div class="container">
    <form action="/update/education" method="post" class="row">
        <div class="col-4 d-flex flex-column">
        </div>
        <div class="col-4 d-flex flex-column">
            <input name="edu_id" value="<?php echo $variables[0]['id'] ?>" hidden>

            <h2 class="align-self-center"><span class="badge bg-info">Education</span></h2>
            <div class="input-group flex-nowrap m-2">
                <span class="input-group-text" id="addon-wrapping">Name</span>
                <input type="text" class="form-control" name="school" value="<?php echo $variables[0]['school_name'] ?>">
            </div>
            <div class="input-group flex-nowrap m-2">
                <span class="input-group-text" id="addon-wrapping">Faculty</span>
                <input type="text" class="form-control" name="faculty" value="<?php echo $variables[0]['faculty'] ?>">
            </div>
            <div class="input-group flex-nowrap m-2">
                <span class="input-group-text" id="addon-wrapping">Field of Study</span>
                <input type="text" class="form-control" name="study_field" value="<?php echo $variables[0]['study_field'] ?>">
            </div>
            <div class="input-group flex-nowrap m-2">
                <select class="form-select" aria-label="Default select example" name="study_status" id="study_status"
                        onchange="changeStudyEnd()">
                    <option selected value=
                    "<?php echo $variables[0]['status'] ?>"><?php echo $variables[0]['status'] ?></option>
